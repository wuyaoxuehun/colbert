...
from colbert.training.training_utils import SequentialDistributedSampler, split_parameters, MAverage, distributed_concat, get_t5_optimizer, get_epoch_scheduler
    ...
    for epoch in tqdm(range(config.epoch)):
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            with amp.context():
                ...
                losses = model(batch, train_dataset, neg_weight_mask=neg_weight_mask)
                # if (step) % 16 == 111:
                #     idx = 0
                #     # print(scores[idx])
...
def eval_retrieval(args, colbert_qa=None):
    ...
    neg_weight_mask = None
    if schedule:
        neg_weight_mask, _ = get_epoch_scheduler(args.epoch, args, eval_p_num=eval_p_num)
    for step, batch in enumerate(epoch_iterator):
        ...
        with torch.no_grad():
            with amp.context():
                losses = \
                    model(batch, train_dataset, is_evaluating=True, merge=False, doc_enc_training=True, eval_p_num=eval_p_num, pad_p_num=0, neg_weight_mask=neg_weight_mask)
                total_loss = sum(losses)
                losses = [total_loss] + list(losses)
                if (step) % 25 == 111:
                    idx = 0
    ...
    # torch.distributed.barrier()
    if args.distributed:
        losses = [distributed_concat(tensor=_.unsqueeze(0), num_total_examples=None).sum() / eval_steps
                  for _ in tr_losses]
...
def eval_retrieval_for_model(args):
    ...
    for step, batch in enumerate(epoch_iterator):
        ...
        with amp.context(), torch.no_grad():
            ...
            model(batch, train_dataset, is_testing_retrieval=True, eval_p_num=10)
            # pass
            output_data += batch
            continue
