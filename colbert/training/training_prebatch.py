...
from colbert.base_config import pos_num, neg_num, pn_num, pretrain, ColBert
from colbert.training.losses import binary_listnet
...
def train(args):
    ...
    labels = torch.zeros((args.bsize, args.bsize * pn_num), dtype=torch.long, device=DEVICE)
    ...
    for epoch in range(args.epoch):
        ...
        for batch_idx, BatchSteps in batch_iterator:
            ...
            for queries, passages in BatchSteps:
                ...
                d_ids, d_mask, d_word_mask = passages
                labels = labels.to(DEVICE)
                with amp.context():
                    if args.mask_q == 1:
                        scores = colbert((q_ids, q_mask), (d_ids, d_mask), q_mask=q_word_mask, d_mask=d_word_mask)
                    else:
                        scores = colbert((q_ids, q_mask), (d_ids, d_mask), q_mask=None, p_mask=None)
                    loss = criterion(scores, labels[:scores.size(0), :scores.size(0) * pn_num])
            ...
            if args.rank < 1:
                batch_iterator.set_postfix(avg_loss='%.4f' % avg_loss, elapsed='%.4d' % elapsed,
