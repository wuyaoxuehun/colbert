...
class CBQADataset(Dataset):
    if use_word and False:
        ...
        doc_pre_tensorize = torch.load(train_dev_doc_pre_tensorize_file)
    ...
    def merge_to_reader_input(self, batch_examples, batch_paras, extra):
        para_idx = 0
        assert len(batch_paras) == len(batch_examples)
        for example in batch_examples:
            example['contexts'] = batch_paras[para_idx]
            example['extra'] = extra[para_idx]
            para_idx += 1
    ...
    def tokenize_for_train_retriever(self, batch: List[Dict], padded_negs=None, eval_p_num=None, is_evaluating=False):
        ...
        # if padded_negs and False:
        #     docs += padded_negs
        if use_word and use_pre_tensorize:
            tensorized_docs = []
            for doc in docs:
                tensorized_docs.append(self.doc_pre_tensorize[doc['idx']])
