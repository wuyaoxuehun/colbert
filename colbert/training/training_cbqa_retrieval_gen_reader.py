...
from colbert.modeling.colbert_list_qa_gen_reader import *
...
def train(args):
    ...
    config = args
    self = args
    loss_names = ["tr_loss", "reader_loss", "rank_loss"]
    ...
    for epoch in tqdm(range(config.epoch)):
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            with amp.context():
                ...
                outputs = model(batch, train_dataset, neg_weight_mask=neg_weight_mask, args=args)
                # if (step) % 16 == 111:
                #     idx = 0
                #     # print(scores[idx])
            ...
            if self.rank <= 0:
                ...
                # postfix = {loss_name: '%.4f' % (loss.get_average() * args.gradient_accumulation_steps) for loss_name, loss in zip(loss_names, tr_losses)}
            ...
            if (step + 1) % global_log_step == 0:
                eval_loss = eval_retrieval_reader(args, model)
                # eval_loss = 0
                if save_every_eval or eval_loss >= best_metrics:
                    best_metrics = max(eval_loss, best_metrics)
                    if self.rank == 0:
                    ...
                    for loss_name, avg_loss in zip(loss_names, tr_losses):
                        # logger.info(f"{loss_name} = %s", avg_loss.get_average() * args.gradient_accumulation_steps)
                    ...
                    eval_retrieval_reader(args, model, test_task)
                if self.distributed:
                    distributed.barrier(self.rank)
...
def eval_retrieval(args, colbert_qa=None):
    ...
    loss_names = ["eval_tr_loss", "eval_re_loss"]
    ...
    return eval_losses[loss_names[0]]


def eval_retrieval_reader(args, colbert_qa=None, task=dev_task):
    # setseed(12345 + args.rank)
    if colbert_qa is None:
        # colbert_config['checkpoint'] = f"output/webq/{save_model_name}/pytorch.bin"
        exit()
    ...
    tokenizer = CostomTokenizer.from_pretrained(pretrain)
    train_dataset = CBQADataset(task, tokenizer=tokenizer, doc_maxlen=colbert_config['doc_maxlen'],
                                query_maxlen=colbert_config['query_maxlen'], reader_max_seq_length=reader_config['max_seq_length'], mode='dev')
    # t_total = len(train_dataset) // args.gradient_accumulation_steps * args.epoch
    # train_sampler = SequentialSampler(train_dataset)
    train_sampler = DistributedSampler(train_dataset) if args.distributed else SequentialSampler(train_dataset)
    ...
    eval_steps = 0
    logger.info("doing evaluating")
    loss_names = ["eval_tr_loss", "eval_reader_loss", "eval_rank_loss"]
    tr_losses = [torch.tensor(0.0).cuda() for _ in range(len(loss_names))]
    neg_weight_mask = None
    # if schedule:
    neg_weight_mask, _ = get_epoch_scheduler(args.epoch, args, eval_p_num=eval_p_num, is_evaluating=True)
    ...
    for step, batch in enumerate(epoch_iterator):
        ...
        #     epoch_iterator.set_postfix(postfix)
    # torch.distributed.barrier()
    all_outputs = [torch.cat(_, dim=0) for _ in list(zip(*all_outputs))]
    if args.distributed:
        losses = [distributed_concat(tensor=_.unsqueeze(0), num_total_examples=None).mean() / eval_steps
                  for _ in tr_losses]
        all_outputs = [distributed_concat(tensor=_, num_total_examples=None)
                       for _ in all_outputs]
    all_preds, all_labels = all_outputs
    pred_labels = all_preds.cpu().numpy().argmax(-1)
    accuracy = (pred_labels == (all_labels.cpu().numpy())).mean()
    logger.info("accuracy = %.4f", accuracy)

    eval_losses = {loss_name: (float(loss)) for loss_name, loss in zip(loss_names, losses)}
    logger.info(f"task = %s", task)
    for k, v in eval_losses.items():
        logger.info(f"{k} = %s", v)
    ...
    # p_num, padded_p_num = t_pnum, tpad_pnum
    if training_rank:
        return - eval_losses['eval_tr_loss']
    return accuracy


def eval_retrieval_for_model_(args):
    # setseed(12345 + args.rank)
...
def eval_retrieval_for_model(args):
    ...
    eval_res = eval_metric_for_data(train_dataset.data)
