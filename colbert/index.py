...
def main():
    ...
    parser.add_argument('--chunksize', dest='chunksize', default=100.0, required=False, type=float)   # in GiBs
    ...
    with Run.context():
        ...
        if os.path.exists(args.index_path):
            # overwrite = input('overwrite existing index?')
            # if overwrite == 'no':
            #     assert not os.path.exists(args.index_path), args.index_path
            print('overwring existing index')
