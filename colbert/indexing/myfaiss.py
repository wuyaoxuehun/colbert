...
def load_sample(samples_paths, sample_fraction=None):
    ...
    for filename in samples_paths:
        ...
        if sample_fraction:
            ...
            part = part[np.random.choice(list(range(part.size(0))), size=(int(part.size(0) * sample_fraction),), replace=False)]
        sample.append(part)

    sample = torch.cat(sample).cpu().float().numpy()
...
def index_faiss(args):
    ...
    for slice_idx, part_offset in enumerate(range(0, len(parts), num_parts_per_slice)):
        ...
        print_message("overwrting existing index path")
