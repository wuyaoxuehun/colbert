...
# from colbert.modeling.cpy.hello_world import span_mean
...
class ColBERT_List_qa(nn.Module):
    def __init__(self, config, colbert_config, reader_config, load_old=True):
        ...
        #                                        dim=colbert_config['dim'],
        # similarity_metric = colbert_config['similarity'])
        # self.span_len_embedding = nn.Embedding(64, dim)
        self.span_len_embedding = torch.nn.Parameter(torch.log(torch.arange(100).float() + 2), requires_grad=False)
        # self.span_len_embedding = torch.nn.Parameter(torch.ones(100).float(), requires_grad=False)
        # print(self.span_len_embedding)
        # self.span_len_embedding.weight = self.span_len_embedding.weight.data.fill_(1)
        # self.span_len_embedding = span_len_embedding.weight.fill_(1.0)
    ...
    def get_representation(self, t, active_indices, multiply_span_len=True):
        if use_word:
    ...
    def query(self, input_ids, attention_mask, active_indices=None, decoder_labels=None, **kwargs):
        ...
        # Q = span_mean(Q, active_indices)
        # Q = self.linear(Q)
        # if l2norm:
        #     Q = torch.nn.functional.normalize(Q, p=2, dim=2)
        Q = self.get_representation(Q, active_indices)
        if decoder_labels is not None:
            assert False
            if teacher_aug:
                if l2norm:
    ...
    def doc(self, input_ids, attention_mask, active_indices=None, **kwargs):
        ...
        # D = span_mean(D, active_indices)

        # t = attention_mask[0]
        # print(t.sum(-1).max(0)[0], active_indices[0])
        # D = self.linear(D)
        D = self.get_representation(D, active_indices, multiply_span_len=False)
        # print("123", D[-3, 0, ...])
        # if l2norm:
    ...
    def forward(self, batch, train_dataset, is_evaluating=False, merge=False, doc_enc_training=False, eval_p_num=None, is_testing_retrieval=False, pad_p_num=None):
        ...
        d_ids, d_attention_mask, d_active_indices, d_active_padding = [(_.to(DEVICE) if type(_) != list else _)
                                                                       for _ in
