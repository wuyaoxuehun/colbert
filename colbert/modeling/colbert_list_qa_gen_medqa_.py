...
class ColBERT_List_qa(BaseModel):
    ...
    def get_mrr(self, scores, positive_idx_per_question):
    ...
    def forward(self, batch, train_dataset, is_evaluating=False, merge=False, doc_enc_training=True,
        ...
        # else:
        # D, D_output = self.doc(d_ids, d_attention_mask, d_active_indices, with_word_weight=doc_word_weight, dpr=True, output_ori=True)
        D = self.doc(d_ids, d_attention_mask, d_active_indices, with_word_weight=doc_word_weight, dpr=False, output_ori=False)
        ...
        # scores = scores.cuda()
        if is_evaluating:
            # return self.get_mrr(pred_scores, positive_idx_per_question), self.get_mrr(pred_esim_scores, positive_idx_per_question)
        ...
        # cur_retriever_esim_loss = retriever_criterion(scores=pred_esim_scores / score_temperature, positive_idx_per_question=positive_idx_per_question)
