...
def cache_function(parts=None, max_seq_length=None, weights=None, *args, **kwargs):
...
class CostomTokenizer(BertTokenizer):
    ...
    def tokenize_multiple_parts(self, parts, max_seq_length, weights=None, generation_ending_token_idx=None, keep=()):
        ...
        word_pos0_mask = [1, 0] + reduce(lambda a, b: a + ['[SEP]'] + b, word_pos0_mask) + [0] + \
    ...
    def tokenize_q_noopt_segmented_dict(self, batch_examples, max_seq_length, answer_max_seq_lenght=64):
        ...
        for t in batch_examples:
            ...
            input_ids, attention_mask, word_pos0_mask, cur_segments = \
                self.tokenize_multiple_part(enum, max_seq_length=max_seq_length, weights=part_weight[:2], generation_ending_token_idx=3, keep=[])
            ...
            input_ids, attention_mask, word_pos0_mask, cur_segments = \
                self.tokenize_multiple_part([answer], max_seq_length=answer_max_seq_lenght, generation_ending_token_idx=3, keep=[])
    ...
    def tokenize_d_segmented_dict(self, batch_text, max_seq_length, tqdm_enable=False):
        ...
        for t in batch_text:
            ...
            if key in self.doc_cache:
                ...
                self.truncate_seq(tokens_list=[doc_tokens], max_len=max_seq_length - 3)
                part_len = len(doc_tokens)
                word_pos0_mask = word_pos0_mask[:part_len]
                cur_segments = cur_segments[:len([_ for _ in word_pos0_mask if _ != 0])]
                ...
                word_pos0_mask = [1, 0] + word_pos0_mask + [0]
